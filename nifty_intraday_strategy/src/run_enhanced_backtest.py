"""
Main script to run the enhanced Nifty intraday backtest with all indicators.
"""
...
def main():
    ...
    print("\nInitializing enhanced backtester...")
    backtester = EnhancedBacktester(config)
    
    # Run backtest
    print("\n" + "="*70)
    print("RUNNING ENHANCED BACKTEST WITH ALL INDICATORS")
    print("="*70 + "\n")
    ...
    if save_results:
        ...
        if len(trades_df) > 0:
            trades_df.to_csv(results_dir / 'enhanced_trades.csv', index=False)
            print(f"\nTrades saved to {results_dir / 'enhanced_trades.csv'}")
        
        equity_curve_df.to_csv(results_dir / 'enhanced_equity_curve.csv', index=False)
        print(f"Equity curve saved to {results_dir / 'enhanced_equity_curve.csv'}")
        
        # Save metrics
        import json
        with open(results_dir / 'enhanced_metrics.json', 'w') as f:
            json.dump(metrics, f, indent=2)
        print(f"Metrics saved to {results_dir / 'enhanced_metrics.json'}")
    else:
        print("Skipping file saves (disabled in config)")
    
    print("\n✅ Enhanced backtest completed successfully!")
    print("\nThe enhanced strategy uses:")
    print("  ✓ Candlestick patterns (Marubozu, Hammer, Doji, Engulfing, etc.)")
    print("  ✓ Technical indicators (RSI, MACD, Chaikin, Moving Averages)")
    print("  ✓ Chart patterns (Head & Shoulders, Double/Triple tops/bottoms)")
    print("  ✓ Price action tools (Pivot Points, Fibonacci Retracement)")
    print("  ✓ Volatility measures (ATR, Standard Deviation, Volatility Ratio)")
    print("  ✓ Multiple timeframe analysis (15min, 1H, 1D)")
